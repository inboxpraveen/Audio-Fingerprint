...
1. The field defaults on :class:`Settings`.
2. The selected profile: ``development``, ``production`` or ``testing``.
3. Environment variables with the ``AUDIOFP_`` prefix. These can also come
...
Every option is documented in ``docs/CONFIGURATION.md``. That file is generated
from the field metadata below, so the two can't drift apart.
"""
...
# bump this when an algorithm change (hash layout, STFT framing, peak picking
# rules) makes stored fingerprints incompatible with freshly computed ones.
# Parameter changes are already covered by the signature.
...
class Settings:
    ...
    Field metadata keys: ``help`` is the text shown in the docs and ``secret``
    marks a value that is never printed. ``fingerprint`` marks a parameter that
    goes into the fingerprint signature. ``env=False`` stops a field being set
    from the environment.
    """

    # ------------------------------------------------------------------ general
    profile: str = field(default="development", metadata=_doc("Configuration profile: development, production or testing."))
    debug: bool = field(default=False, metadata=_doc("Enable Flask debug mode. For development only, never in production."))
    ...
    chunk_seconds: float = field(
        default=30.0, metadata=_doc("Audio is decoded and fingerprinted in chunks of this many seconds, so memory use stays flat on hour-long files.")
    )
    max_query_seconds: float = field(
        default=3600.0,
        metadata=_doc(
            "Query audio longer than this is cut off and the response sets query.truncated. The default fits an hour-long call recording. Raise it for longer material."
        ),
    )
    ...
    peak_neighborhood_size: int = field(
        default=20,
        metadata=_doc(
            "Size of the local-maximum window used to pick spectral peaks, in frames x frequency bins. A smaller window gives denser fingerprints.",
            fingerprint=True,
        ),
    )
    min_amplitude: float = field(
        default=10.0, metadata=_doc("Minimum linear STFT magnitude for a peak to be kept. Filters out silence and the noise floor.", fingerprint=True)
    )
    fan_value: int = field(
        default=10,
        metadata=_doc(
            "Number of following peaks each anchor peak is paired with. Higher values produce more hashes, which helps with noisy audio and costs more storage.",
            fingerprint=True,
        ),
    )
    min_hash_time_delta: int = field(
        default=0,
        metadata=_doc(
            "Minimum frame distance between paired peaks. At 0, simultaneous peaks such as the harmonics of a chord are paired too, and those pairs hold up well in noise. Set 1 to leave them out on very repetitive tonal material.",
            fingerprint=True,
        ),
    )
    max_hash_time_delta: int = field(
        default=200, metadata=_doc("Maximum frame distance between paired peaks. 200 frames is about 9 s at the default rate.", fingerprint=True)
    )
    ...
    min_confidence: float = field(default=0.02, metadata=_doc("Minimum fraction of query hashes aligned at the best offset, between 0 and 1."))
    min_peak_ratio: float = field(
        default=12.0,
        metadata=_doc(
            "Minimum ratio between the aligned count at the best offset and the mean count across all offsets, a measure of peak sharpness. A chance match is flat and a real match spikes."
        ),
    )
    offset_tolerance_frames: int = field(
        default=1, metadata=_doc("Adjacent offset bins within +/- this many frames are merged when scoring. This absorbs jitter in where a clip starts.")
    )
    max_occurrences_per_track: int = field(default=25, metadata=_doc("Cap on the occurrences reported per track in 'occurrences' mode."))
    max_rows_per_hash: int = field(
        default=2000,
        metadata=_doc(
            "Query hashes that occur more than this many times in the library are ignored as 'stop words', typically hold-music loops and test tones. 0 disables the cap."
        ),
    )
    max_search_votes: int = field(
        default=5_000_000,
        metadata=_doc(
            "Upper bound on the offset votes examined per search. When a query would exceed it, the most common hashes are dropped first. This keeps memory bounded on very repetitive material."
        ),
    )
    ...
    sqlite_mmap_mb: int = field(default=256, metadata=_doc("SQLite memory-mapped I/O window in MB. 0 disables it."))
    sqlite_write_batch_rows: int = field(
        default=2_000_000,
        metadata=_doc(
            "Fingerprint rows buffered in memory before they are written to SQLite in one sorted transaction. Each row takes about 16 bytes. 0 writes every track immediately. Buffered tracks are still searchable, and the buffer is flushed at the end of every indexing run and on shutdown."
        ),
    )
    sqlite_track_index: bool = field(
        default=False,
        metadata=_doc(
            "Maintain a secondary index on track_ref so that deleting tracks stays fast on very large libraries. It costs about as much disk as the fingerprint table itself."
        ),
    )
    ...
    fingerprint_compat: str = field(
        default="strict",
        metadata=_doc(
            "What to do when the database was built with different fingerprint parameters. strict refuses to start, warn logs it and carries on, ignore does nothing."
        ),
    )
    ...
    keep_failed_uploads: bool = field(default=False, metadata=_doc("Keep uploaded files on disk when indexing fails. Useful when debugging."))
    dedupe: str = field(
        default="content",
        metadata=_doc(
            "How duplicates are detected when indexing. content compares a SHA-256 of the file bytes, path compares the file path, none turns it off."
        ),
    )
    index_roots: list[str] = field(
        default_factory=list,
        metadata=_doc(
            "Comma-separated list of directories the server may index through the API. When empty, development allows any path and production allows none."
        ),
    )
    allow_directory_indexing: bool = field(default=True, metadata=_doc("Expose POST /tracks/index-directory. Turn it off to allow uploads only."))

    # ------------------------------------------------------------------ jobs
    index_workers: int = field(default=0, metadata=_doc("Threads that fingerprint files concurrently. 0 means min(4, CPU count)."))
    max_concurrent_jobs: int = field(default=2, metadata=_doc("How many indexing jobs may run at the same time. They all share the index worker pool."))
    job_history_limit: int = field(default=200, metadata=_doc("Finished jobs kept in memory and on disk."))
    job_max_errors: int = field(default=500, metadata=_doc("Per-file errors kept for each job."))
    ...
    api_key: str = field(
        default="", metadata=_doc("If set, every /api/v1 request other than /health must send it as X-API-Key or Authorization: Bearer.", secret=True)
    )
    cors_origins: list[str] = field(
        default_factory=list,
        metadata=_doc(
            "Comma-separated list of origins allowed to call the API from a browser. '*' allows any origin. Empty means same-origin only, which is all the bundled UI needs. Never use '*' on a server without an API key, because any website could then drive the API."
        ),
    )
    ...
    log_file: str = field(
        default="auto",
        metadata=_doc(
            "Path of the rotating log file. 'auto' means <data_dir>/logs/audiofp.log in the production profile and console only in the others. 'none' or an empty value means console only."
        ),
    )
    ...
    def public_dict(self) -> dict[str, Any]:
        """Settings safe to expose, with secrets redacted."""
        out: dict[str, Any] = {}
    ...
    def load(cls, profile: str | None = None, env: dict[str, str] | None = None, dotenv: bool = True, **overrides: Any) -> Settings:
        """Build settings from defaults, profile, environment and explicit overrides, in that order."""
        ...
        for f in fields(cls):
            ...
            if raw == "" and str(f.type) not in ("str", "<class 'str'>") and not str(f.type).startswith("list"):
                continue  # an empty value only means something for text and list settings, e.g. AUDIOFP_LOG_FILE= turns the file off
...
def describe_settings() -> list[dict[str, Any]]:
    """Field metadata as rows, for the docs tables and ``audiofp config --describe``."""
    rows = []
